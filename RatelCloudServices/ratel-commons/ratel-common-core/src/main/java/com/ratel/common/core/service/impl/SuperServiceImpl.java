package com.ratel.common.core.service.impl;
...
import com.ratel.common.core.exception.IdempotencyException;
import com.ratel.common.core.lock.DistributedLock;
import com.ratel.common.core.lock.LockException;
import com.ratel.common.core.lock.RCLock;
...
public class SuperServiceImpl<M extends BaseMapper<T>, T> extends ServiceImpl<M, T> implements SuperService<T> {
    ...
    public boolean saveIdempotency(T entity, DistributedLock locker, String lockKey, Wrapper<T> countWrapper, String msg) throws Exception {
        ...
        try (
                RCLock lock = locker.tryLock(lockKey, 10, 60, TimeUnit.SECONDS);
        ) {
            if (lock != null) {
                //判断记录是否已存在
